  ...
  return (
    <div id="store" className="min-h-screen flex flex-col items-center justify-center">
      <Helmet>
        ...
        <meta name="author" content="Bakerz Bite" />
      </Helmet>

      <header className="bg-gradient-to-r text-white py-16 px-8 shadow-lg w-full text-center">
        <h1 className="text-4xl sm:text-5xl header font-extrabold tracking-tight">Gaming Accessories Store</h1>
      </header>
            ...
            <button
              ...
              className={`px-6 py-2 font-semibold rounded-full transition transform hover:scale-105 shadow-sm ${
                selectedCategory === category
                  ...
                  : 'bg-white/10 text backdrop-blur-md text-white hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600'
              }`}
            >
              {category}
            </button>
          ...
          {finalProducts.length > 0 ? (
            finalProducts.map((product) => (
              ...
              >
                ...
                <div className="p-6">
                  <h4 className="text-2xl font-bold text text-gray-900 mb-3">{product.name}</h4>
                  <p className="text-gray-700 text text-sm mb-4 opacity-90 line-clamp-2">{product.description}</p>
